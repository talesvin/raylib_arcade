...
Ball updateBall(){
    Ball ball = {14, screenWidth/2, screenHeight/2, ballSpeed(-8, 8, true), ballSpeed(-8, 8, false)};
    return ball;
}
    ...
    while(!WindowShouldClose()){
        BeginDrawing();
            ...
            if(IsKeyDown(KEY_W) && player1.posy > 0){
                player1.posy -= 10;
            }else if(IsKeyDown(KEY_S) && player1.posy + player1.height < screenHeight){
                player1.posy += 10;
            }
            ...
            if(IsKeyDown(KEY_UP) && player2.posy > 0){
                player2.posy -= 10;
            }else if(IsKeyDown(KEY_DOWN) && player2.posy + player2.height < screenHeight){
                player2.posy += 10;
            }
            ...
            int paddle_height1 = ball.posy >= player1.posy && ball.posy <= player1.posy + player1.height;
            int paddle_height2 = ball.posy >= player2.posy && ball.posy <= player2.posy + player2.height;

            // collision left paddle
            if(ball.x_speed <= 0 && (ball.posx - (ball_paddle_diff1) <= player1.posx) && paddle_height1){
                ball.x_speed *= -1; // revert the ball direction
                ball.x_speed *= 1.5; // increase the ball speed
            }

            // collision right paddle
            if(ball.x_speed >= 0 && (ball.posx + (ball_paddle_diff2) >= player2.posx) && paddle_height2){
                ball.x_speed *= 1.5;
                ball.x_speed *= -1;
            }
